...
const privatekey =
  "840f7cdb2ea1dc93604204d3b53be769ef1c7faf";
const ts = new Date().getTime();
const stringToHash = ts + privatekey + publickey;
const hash = md5(stringToHash);
const baseUrlforcharacters =
  "https://gateway.marvel.com:443/v1/public/characters";
const baseUrlforcomics =
  "https://gateway.marvel.com:443/v1/public/comics";
const baseUrlforstories =
...
app.get("/characters/:id", async (req, res) => {
  try {
    if (isNaN(req.params.id))
      throw new Error(`ID must be in Number`);
    characterID = req.params.id;
    const cachedData = await client.hGet(
      "CharacterForID",
      characterID
    );
  ...
  async (req, res) => {
    try {
      if (isNaN(req.params.pagenum))
        throw new Error(`PageNum must be in Number`);
      if (!req.params.pagenum)
        throw new Error(`No Page num`);
      ...
      try {
        ...
        if (finalData.data.results.length == 0)
...
app.get(
  "/api/characters/search/:searchTerm",
  async (req, res) => {
    try {
